...
function openDB() {
  return new Promise((resolve,reject)=>{
    ...
    req.onupgradeneeded = ()=> req.result.createObjectStore('registros',{keyPath:'DNI'});
...
document.getElementById('btnDNI').onclick = async ()=>{
  ...
  document.getElementById('results').innerHTML = res
    ? `<table><thead><tr>${Object.keys(res).map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody><tr>${Object.values(res).map(v=>`<td>${v}</td>`).join('')}</tr></tbody></table>`
    : '<p>No encontrado.</p>';
};
...
document.getElementById('btnName').onclick = async ()=>{
  ...
  const f = all.filter(o=> Object.values(o).some(v=>v.toString().toLowerCase().includes(term)));
  console.log('📋',f.length,'matches');
  document.getElementById('results').innerHTML = f.length
    ? `<table><thead><tr>${Object.keys(f[0]).map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${f.map(r=>`<tr>${Object.values(r).map(v=>`<td>${v}</td>`).join('')}</tr>`).join('')}</tbody></table>`
    : '<p>No encontrado.</p>';
};
